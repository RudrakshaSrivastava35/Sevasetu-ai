...
import { Trash2, RotateCcw, Clock, Trash } from 'lucide-react';
import { motion } from 'motion/react';
import { formatDistanceToNow, parseISO } from 'date-fns';
import toast from 'react-hot-toast';
import type { Need } from '../../../types';

export default function VolunteerRecycleBin() {
  const { profile } = useAuth();
  const [deletedTasks, setDeletedTasks] = useState<Need[]>([]);
  ...
  async function handleRestore(id: string) {
    ...
    try {
      ...
      if (error) throw error;
      setDeletedTasks(deletedTasks.filter(n => n.id !== id));
  ...
  async function handlePermanentDelete(id: string) {
    if (!confirm('Permanently delete this task from your list?')) return;
    setProcessing(id);
    try {
      // For volunteers, permanent delete might just mean unassigning if we want to be safe, 
      // but the prompt says "DELETE row from database". 
      // However, volunteers shouldn't be able to delete "needs" entirely, only NGOs/Creators should.
      // But for the sake of the "Recycle Bin hackathon demo", I'll follow the instruction.
      const { error } = await supabase
        .from('needs')
        .delete()
        .eq('id', id);
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === deletedTasks.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(deletedTasks.map(n => n.id));
    }
  };

  async function handleBulkAction(action: 'restore' | 'delete') {
    if (selectedIds.length === 0) return;
    if (action === 'delete' && !confirm(`Permanently delete ${selectedIds.length} tasks?`)) return;
    ...
    try {
      if (action === 'restore') {
        const { error } = await supabase
          ...
          .in('id', selectedIds);
        if (error) throw error;
        toast.success(`Restored ${selectedIds.length} tasks`);
      } else {
        const { error } = await supabase
          .from('needs')
          .delete()
          .in('id', selectedIds);
        if (error) throw error;
        toast.success(`Deleted ${selectedIds.length} tasks permanently`);
      }
      setDeletedTasks(deletedTasks.filter(n => !selectedIds.includes(n.id)));
      setSelectedIds([]);
    } catch (error) {
      setProcessing(null);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">Deleted tasks will be held for 7 days.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                <th className="px-6 py-4 w-10">
                  <input 
                    type="checkbox" 
                    checked={deletedTasks.length > 0 && selectedIds.length === deletedTasks.length}
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">Loading bin...</td>
                </tr>
